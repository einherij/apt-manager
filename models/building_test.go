...
func testBuildingToManySetOpApartments(t *testing.T) {
	var err error
	...
	err = a.SetApartments(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetApartments(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Apartments[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Apartments[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testBuildingToManyRemoveOpApartments(t *testing.T) {
	var err error
	...
	err = a.AddApartments(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveApartments(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Apartments) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Apartments[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Apartments[0] != &e {
