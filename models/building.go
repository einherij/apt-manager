...
// SetApartments removes all previously related items of the
// building replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Building's Apartments accordingly.
// Replaces o.R.Apartments with related.
// Sets related.R.Building's Apartments accordingly.
func (o *Building) SetApartments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Apartment) error {
	query := "update \"apartment\" set \"building_id\" = null where \"building_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Apartments {
	...
	return o.AddApartments(ctx, exec, insert, related...)
}

// RemoveApartments relationships from objects passed in.
// Removes related items from R.Apartments (uses pointer comparison, removal does not keep order)
// Sets related.R.Building.
func (o *Building) RemoveApartments(ctx context.Context, exec boil.ContextExecutor, related ...*Apartment) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Apartments {
			if rel != ri {
				continue
			}

			ln := len(o.R.Apartments)
			if ln > 1 && i < ln-1 {
				o.R.Apartments[i] = o.R.Apartments[ln-1]
			}
			o.R.Apartments = o.R.Apartments[:ln-1]
			break
		}
	}
